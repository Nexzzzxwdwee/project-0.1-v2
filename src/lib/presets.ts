...
/**
 * Get all day plans that reference a specific preset (single batch query)
 */
export async function getDayPlansReferencingPreset(presetId: PresetId): Promise<DayPlan[]> {
  const storage = getStorage();
  return storage.getDayPlansByPresetId(presetId);
}
 ...
 * Fetches all sealed summaries in one batch, then computes streak in-memory.
 */
export async function getStreak(today?: string): Promise<number> {
  ...
  // Single query — already sorted by date descending
  const allSummaries = await getAllSealedDaySummaries();

  // Build a set of dates that qualify (sealed + 100% operator score)
  const perfectDates = new Set(
    allSummaries
      .filter((s) => s.isSealed && s.operatorTotal > 0 && s.operatorPct === 100)
      .map((s) => s.date)
  );
  ...
  while (perfectDates.has(currentDate)) {
    streak++;
