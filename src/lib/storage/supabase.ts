  ...
  return {
    ...
    async savePresets(presets: Record<PresetId, Preset>): Promise<void> {
      ...
      const rows = Object.values(presets).map((preset) => ({
        id: preset.id,
        user_id: userId,
        name: validateText(preset.name, 'Preset name', 200),
    ...
    async getDayPlansByPresetId(presetId: PresetId): Promise<DayPlan[]> {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', userId)
        .eq('active_preset_id', presetId);

      if (error) {
        console.error('Failed to fetch day plans by preset:', error);
        throw error;
      }
      ...
      return (data || []).map((row: any) => ({
        ...
        presetUpdatedAt: row.preset_updated_at,
        items: (row.items || []) as DayPlanItem[],
        archived: (row.archived || []) as DayPlanItem[],
    ...
    async saveGoals(goals: Goal[]): Promise<void> {
      ...
      const rows = goals.map((goal) => ({
        id: goal.id,
        user_id: userId,
        text: validateText(goal.text, 'Goal text', 1_000),
        tag: goal.tag ? validateText(goal.tag, 'Goal tag', 100) : null,
        done: goal.done,
        done_at: goal.doneAt,
